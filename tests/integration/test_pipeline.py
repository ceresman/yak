...
def test_complete_pipeline(sample_data):
    ...
    # Create sample phenotype data with population information and trait values
    ...
    assert isinstance(selection_stats, pd.DataFrame)

    snp_results = snp_analyzer.evaluate_trait_snps(
        genotypes,
        pd.DataFrame({'trait_value': [0.8, 0.3]}, index=genotypes.index)
    )
    assert isinstance(snp_results, pd.DataFrame)

    # Calculate diversity indices
    diversity_indices = diversity_analyzer.calculate_diversity_indices(
        genotypes, phenotypes['population'])
    assert isinstance(diversity_indices, pd.DataFrame)
