    ...
    def evaluate_trait_snps(self, genotypes: pd.DataFrame,
        ...
        try:
            ...
            for snp in genotypes.columns:
                # Calculate allele frequencies by trait
                freq_by_trait = {}
                for trait in phenotypes['trait'].unique():
                    trait_mask = phenotypes['trait'] == trait
                    freq_by_trait[trait] = genotypes.loc[trait_mask, snp].mean() / 2
